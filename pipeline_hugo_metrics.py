...
# -*- coding: UTF-8 -*-
'''
@Project ：pure_pathway_2 
@File    ：pipeline_hugo_metrics.py
@Author  ：yang
@Date    ：2023/5/7 11:29 
'''
...
def get_parser():
    ...
    parser.add_argument(
        "--input",
        default="/root/lk/pathway_retinanet-pathway_retinanet_pairing/img/result.jpg",
        # nargs="+",
    ...
    parser.add_argument(
        "--output",
        # default="/mnt/data/result/new.png",
...
def reorganize_outputs(el_output, body_output, image_ids, file_names, cfg):
    ...
    if el_model_instances.empty or body_instances.empty:
        ...
        relation_body_instances = pd.DataFrame(
            columns=['image_id', 'file_name', 'category_id', 'bbox', 'score', 'ocr', 'head', 'tail'])
...
def get_ocr(current_image_file, article_gene_list, gene_name_list, data_folder, image_name, relation_body_instances,
    ...
    if len(box_res['box']) == 0:
        relation_body_instances = pd.DataFrame(columns=['image_id', 'file_name', 'category_id', 'normalized_bbox',
                                                        'receptor', 'startor_bbox', 'receptor_bbox',
                                                        'relation_category'])
    ...
    save_slice = False
    ocr_results, coordinates_list = mmocr_without_det(recog='SATRN', img=current_image_file,
                                                      element_bbox=box_res['box'], save_slices=save_slice)
...
def save_visual(current_image_file, processed_el_body_instance, visuals_folder, image_name, ext):
    ...
    for element_idx in range(0, len(processed_el_body_instance)):
        if (processed_el_body_instance.iloc[element_idx]['startor'] != '-') and (
                processed_el_body_instance.iloc[element_idx]['receptor'] != '-'):
            if processed_el_body_instance.iloc[element_idx]['category_id'] == 0:
                if processed_el_body_instance.iloc[element_idx]['score'] >= cfg.element_threshold:
                    if processed_el_body_instance.iloc[element_idx]['startor'] != \
                            processed_el_body_instance.iloc[element_idx]['receptor']:
                        cv2.polylines(img_copy,
                                      [normalize_rect_vertex(processed_el_body_instance.iloc[element_idx]['bbox'],
                                                             image_size)],
                                      isClosed=True, color=(255, 0, 0), thickness=2)
            elif processed_el_body_instance.iloc[element_idx]['category_id'] == 1 and \
                    processed_el_body_instance.iloc[element_idx]['ocr'] != '-' \
    ...
    for i in range(0, len(processed_el_body_instance[(processed_el_body_instance['category_id'] != 1) & (
            processed_el_body_instance['startor'] != '-')
                                                     & (processed_el_body_instance['receptor'] != '-') &
                                                     (processed_el_body_instance['startor'] != None) & (
                                                             processed_el_body_instance['receptor'] != None)])):
        head = processed_el_body_instance[
            (processed_el_body_instance['category_id'] != 1) & (processed_el_body_instance['startor'] != '-')
            & (processed_el_body_instance['receptor'] != '-') &
            (processed_el_body_instance['startor'] != None) & (processed_el_body_instance['receptor'] != None)]['head']
        element_head = head.tolist()
        tail = processed_el_body_instance[
            (processed_el_body_instance['category_id'] != 1) & (processed_el_body_instance['startor'] != '-')
            & (processed_el_body_instance['receptor'] != '-') &
    ...
    with inference_context(el_model.to(device)), inference_context(body_model.to(device)), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            for current_image_file in file_list:
                ...
                # ###
                # print("relation_head_instances", relation_head_instances, "relation_body_instances",
                #       relation_body_instances
                #       , "element_instances", element_instances)
                # relation_path = data_folder + 'relation/'
                # if not os.path.exists(relation_path):
                #     os.makedirs(relation_path)
                #
                # with open('{:s}_o_d_ele.json'.format(os.path.join(relation_path, image_name)),
                #           'w') as output_fp:
                #     element_instances.to_json(output_fp, orient='index')
                # with open('{:s}_o_d_rela_head.json'.format(os.path.join(relation_path, image_name)),
                #           'w') as output_fp:
                #     relation_head_instances.to_json(output_fp, orient='index')
                # with open('{:s}_o_d_rela_body.json'.format(os.path.join(relation_path, image_name)),
                #           'w') as output_fp:
                #     relation_body_instances.to_json(output_fp, orient='index')
                # ###
                ...
                # with open('{:s}_current_relation_body_instances.json'.format(os.path.join(relation_path, image_name)),
                #           'w') as output_fp:
                #     current_relation_body_instances.to_json(output_fp, orient='index')
                # with open('{:s}_current_relation_head_instances.json'.format(os.path.join(relation_path, image_name)),
                #           'w') as output_fp:
                #     current_relation_head_instances.to_json(output_fp, orient='index')
                ...
                results = result[
                    ["image_id", "file_name", "category_id", "bbox", "normalized_bbox", "score", "startor",
                     "startor_bbox",
                     "relation_category",
